 ...
 * @notice Versão corrigida do PredictTheFutureChallenge usando commit-reveal
 * @dev NOTA: Para produção, use Chainlink VRF. Esta é uma versão simplificada.
 *
 * CORREÇÕES APLICADAS:
 * 1. Removido uso de dados públicos de blocos (block.blockhash, now) para aleatoriedade
 * 2. Implementado esquema commit-reveal para garantir aleatoriedade segura
 * 3. Adicionado controle de estado para prevenir múltiplas tentativas do mesmo endereço
...
contract PredictTheFutureChallengeFixed {
     ...
     * @param _answer O número secreto (0-9)
    ...
    /**
     * @notice Faz lock do palpite (equivalente ao lockInGuess original)
     * @param n O número a ser adivinhado (0-9)
     */
